...
# 1. 初始化
rope = CableDynamicsSimSingle()
payload = PayloadDynamicsSimSingle()
...
# 初始化动作
action = torch.zeros(n, 4, device=rope.device)  # [gamma(3), a]
action[:, 3] = 1  # 初始张力加速度
action[:, 0] = 1  # 初始角加速度
action[:, 2] = 0  # 初始角加速度
...
for t in range(steps):
    ...
    R_l = quat_to_rot(payload.state[6:10])  # (3,3)
    input_force_torque = rope.compute_force_torque(R_l)
    payload.rk4_step(input_force_torque)

    payload_pos_traj.append(payload.state[0:3].cpu().numpy())
    payload_rot_traj.append(R_l.cpu().numpy())
    rope_dir_traj.append(rope.dir.cpu().numpy())
    ...
    if t % frame_interval == 0 or t == steps - 1:
        ...
        center = payload.state[0:3].cpu().numpy()
        R = R_l.cpu().numpy()
